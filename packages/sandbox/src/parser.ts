...
export type ExtractLispOp<L> = L extends DefineLisp<infer i, any, any>
	? i
	: never;
export type ExtractLispA<L> = L extends DefineLisp<any, infer i, any>
	? i
	: never;
export type ExtractLispB<L> = L extends DefineLisp<any, any, infer i>
	? i
	: never;
...
const NullLisp = createLisp<None>({
	op: LispType.None,
	a: LispType.None,
	b: LispType.None,
});
...
export type SpreadObject = DefineLisp<
	LispType.SpreadObject,
	LispType.None,
	Lisp
>;
...
export type IncrementBefore = DefineLisp<
	LispType.IncrementBefore,
	Lisp,
	LispType.None
>;
export type IncrementAfter = DefineLisp<
	LispType.IncrementAfter,
	Lisp,
	LispType.None
>;
export type DecrementBefore = DefineLisp<
	LispType.DecrementBefore,
	Lisp,
	LispType.None
>;
export type DecrementAfter = DefineLisp<
...
export type BitUnsignedShiftRight = DefineLisp<
	LispType.BitUnsignedShiftRight,
	Lisp,
	Lisp
>;
export type SmallerEqualThan = DefineLisp<
...
export type SwitchCase = DefineLisp<
	LispType.SwitchCase,
	LispType.None | Lisp,
	Lisp[]
>;
...
export type GlobalSymbol = DefineLisp<
	LispType.GlobalSymbol,
	LispType.None,
	string
>;
export type LiteralIndex = DefineLisp<
	LispType.LiteralIndex,
	LispType.None,
	string
>;
export type StringIndex = DefineLisp<
...
export type Function = DefineLisp<
	...
	string | Lisp[]
>;
export type InlineFunction = DefineLisp<
	LispType.InlineFunction,
	string[],
	string | Lisp[]
>;
export type ArrowFunction = DefineLisp<
...
const semiColon = /^;/;
const insertedSemicolons: WeakMap<
	{ str: string },
	Array<number>
> = new WeakMap();
	...
	for (i = 0; i < part.length && !done; i++) {
		...
		if (quote === '"' || quote === "'" || quote === "`") {
			...
			if ((foundNumber = aNumber.exec(sub))) {
				...
				if (
					char === ";" ||
					(insertedSemis[i + part.start] && !isStart && !lastInertedSemi)
				) {
					if (hasSemiTest) {
						found = [";"];
...
export const setLispType = <T extends readonly string[]>(
	types: T,
	fn: LispCallback<T[number]>,
) => {
	types.forEach((type) => {
		...
		switch (type) {
			...
			case "createObject":
				l = arg.map((str) => {
					...
					let value: Lisp;
					let key: string | Lisp = "";
					funcFound = expectTypes.expFunction.types.function.exec(
						"function " + str,
					);
					if (funcFound) {
...
setLispType(
	...
	(constants, type, part, res, expect, ctx) => {
		const extract = restOfExp(constants, part.substring(res[0].length), [
			/^([^\s.?\w$]|\?[^.])/,
		]);
		ctx.lispTree = lispify(
			constants,
	...
	(constants, type, part, res, expect, ctx) => {
		...
		ctx.lispTree = createLisp<InlineIf>({
			...
			b: createLisp<InlineIfCase>({
				...
				b: lispifyExpr(
					constants,
					part.substring(res[0].length + extract.length + 1),
				),
			}),
		});
	},
...
function extractIfElse(constants: IConstants, part: CodeString) {
	...
	while (
		...
		if (f.startsWith("if")) {
			...
			foundTrue = foundElse.length
				? foundTrue
				: part.substring(0, found.end - part.start);
			break;
		}
		if (!count) {
...
setLispType(["switch"] as const, (constants, type, part, res, expect, ctx) => {
	...
	while ((caseFound = caseTest.exec(statement.toString()))) {
		...
		const cond = restOfExp(
			constants,
			statement.substring(caseFound[0].length),
			[/^:/],
		);
		...
		if (bracketFound) {
			...
			if (!notEmpty.trim().length) {
				...
				while (
					(found = restOfExp(constants, statement.substring(i), [semiColon]))
						.length
				) {
					i +=
						found.length + (statement.char(i + found.length) === ";" ? 1 : 0);
...
setLispType(
	...
	(constants, type, part, res, expect, ctx) => {
		...
		if (type === "dot") {
			...
			const matches = part
				.substring(res[0].length)
				.toString()
				.match(expectTypes.prop.types.prop);
...
setLispType(
	["spreadArray", "spreadObject"] as const,
	(constants, type, part, res, expect, ctx) => {
...
setLispType(
	...
	(constants, type, part, res, expect, ctx) => {
		...
		const args: string[] = res[argPos]
			? res[argPos].replace(/\s+/g, "").split(/,/g)
			: [];
		if (!isArrow) {
		...
		ctx.lispTree = lispify(
			...
			createLisp<Function | InlineFunction | ArrowFunction>({
				...
				a: [isAsync, ...args],
				b: constants.eager
					? lispifyFunction(new CodeString(func), constants)
					: func,
			}),
		);
	},
...
const iteratorRegex =
	/^((let|var|const)\s+)?\s*([a-zA-Z$_][a-zA-Z\d$_]*)\s+(in|of)(?![\w$])/;
setLispType(
	["for", "do", "while"] as const,
	(constants, type, part, res, expect, ctx) => {
		let i = 0;
		...
		switch (type) {
			case "while": {
				...
				body = restOfExp(
					constants,
					part.substring(i + extract.length + 1),
				).trim();
			...
			case "for": {
				...
				if (
					args.length === 1 &&
					(iterator = iteratorRegex.exec(args[0].toString()))
				) {
					if (iterator[4] === "of") {
						(getIterator = lispifyReturnExpr(
							constants,
							args[0].substring(iterator[0].length),
						)),
						...
						beforeStep = lispify(
							constants,
							new CodeString(
								(iterator[1] || "let ") + iterator[3] + " = $$next.value",
							),
							["initialize"],
						);
					} else {
						...
						beforeStep = lispify(
							constants,
							new CodeString(
								(iterator[1] || "let ") + iterator[3] + " = $$keys[$$keyIndex]",
							),
							["initialize"],
						);
					}
			...
			case "do": {
				...
				condition = lispifyReturnExpr(
					...
					restOfExp(
						constants,
						part.substring(
							part.toString().indexOf("(", res[0].length + body.length) + 1,
						),
						[],
						"(",
					),
...
setLispType(["block"] as const, (constants, type, part, res, expect, ctx) => {
	ctx.lispTree = createLisp<Block>({
		op: LispType.Block,
		a: lispifyBlock(
			restOfExp(constants, part.substring(1), [], "{"),
			constants,
		),
		b: LispType.None,
	});
...
const catchReg =
	/^\s*(catch\s*(\(\s*([a-zA-Z$_][a-zA-Z\d$_]*)\s*\))?|finally)\s*\{/;
setLispType(["try"] as const, (constants, type, part, res, expect, ctx) => {
	const body = restOfExp(constants, part.substring(res[0].length), [], "{");
	let catchRes = catchReg.exec(
		part.substring(res[0].length + body.length + 1).toString(),
	);
	let finallyBody;
	let exception = "";
	let catchBody;
	...
	if (catchRes![1].startsWith("catch")) {
		...
		if (
			...
			finallyBody = restOfExp(
				constants,
				part.substring(offset + catchRes[0].length),
				[],
				"{",
			);
		}
	...
	const b = [
		exception,
		lispifyBlock(
			insertSemicolons(constants, catchBody || emptyString),
			constants,
		),
		lispifyBlock(
			insertSemicolons(constants, finallyBody || emptyString),
			constants,
		),
	] as LispItem;
...
setLispType(["new"] as const, (constants, type, part, res, expect, ctx) => {
	...
	if (part.char(i - 1) === "(") {
		...
		let j = 0;
		while (
			(found = restOfExp(constants, argsString.substring(j), [/^,/])).length
		) {
			j += found.length + 1;
	...
	ctx.lispTree = lispify(
		...
		createLisp({
			...
			b: args.map((arg) =>
				lispify(constants, arg, expectTypes.initialize.next),
			),
		}),
	);
});
...
const ofCondition = lispify(
	undefined as any,
	new CodeString("return !$$next.done"),
	["initialize"],
);
const ofStep = lispify(
	undefined as any,
...
const inStart3 = lispify(
	undefined as any,
	new CodeString("let $$keyIndex = 0"),
	["initialize"],
);
...
const startingExpectedWithoutSingle = startingExecpted.filter(
	(r) => r !== "expSingle",
);
...
function lispifyExpr(
	...
	if (expected.includes("expSingle")) {
		if (
			testMultiple(str.toString(), Object.values(expectTypes.expSingle.types))
		) {
	...
	if (expected.includes("initialize")) {
		const defined = expectTypes.initialize.types.initialize.exec(
			subExpressions[0].toString(),
		);
		if (defined) {
			...
			expectTypes.initialize.types.return.exec(subExpressions[0].toString())
		) {
			return lispify(constants, str, expected, undefined, true);
		}
	}
	const exprs = subExpressions.map((str) =>
		lispify(constants, str, expected, undefined, true),
	);
	return createLisp<Expression>({
...
export function lispifyBlock(
	...
	while (
		...
		isInserted = !!(
			str.char(pos + part.length) && str.char(pos + part.length) !== ";"
		);
...
const closingsNoInsertion =
	/^(\})\s*(catch|finally|else|while|instanceof)(?![\w$])/;
//  \w|)|] \n \w = 2                                  // \} \w|\{ = 5
const colonsRegex =
	/^((([\w$\])"'`]|\+\+|--)\s*\r?\n\s*([\w$+\-!~]))|(\}\s*[\w$!~+\-{("'`]))/;
...
export function insertSemicolons(
	constants: IConstants,
	str: CodeString,
): CodeString {
	let rest = str;
	...
	let details: restDetails = {};
	const inserted =
		insertedSemicolons.get(str.ref) || new Array(str.ref.str.length);
	while (
		(sub = restOfExp(
			constants,
			rest,
		...
		if (details.regRes) {
			...
			edge =
				details.regRes[3] === "++" || details.regRes[3] === "--"
					? sub.length + 1
					: sub.length;
			part = rest.substring(0, edge);
			if (b) {
				const res = closingsNoInsertion.exec(
					rest.substring(sub.length - 1).toString(),
				);
				if (res) {
					if (res[2] === "while") {
...
const notDivide =
	/(typeof|delete|instanceof|return|in|of|throw|new|void|do|if)$/;
...
export function extractConstants(
			...
			if (quote) {
				...
				if (!isPossibleDivide || !space.test(char)) {
					if ((isPossibleDivide = possibleDivide.exec(str.substring(i)))) {
						if (
							notDivide.test(str.substring(0, i + isPossibleDivide[1].length))
						) {
	...
	if (comment) {
		if (comment === "*") {
			throw new SyntaxError(
				`Unclosed comment '/*': ${str.substring(commentStart)}`,
			);
		}
	}
	return { str: strRes.join(""), length: i };
...
export default function parse(
	...
	for (const l of constants.literals) {
		l[2] = l.tempJsStrings!.map((js: string) =>
			lispifyExpr(constants, new CodeString(js)),
		);
