...
export interface IExecContext extends IExecutionTree {
	...
	setSubscriptionsGlobal: WeakMap<
		...
		Map<string, Set<(modification: Change) => void>>
	>;
	changeSubscriptionsGlobal: WeakMap<
		SubscriptionSubject,
		Set<(modification: Change) => void>
	>;
...
export const SandboxGlobal = function SandboxGlobal(
	this: ISandboxGlobal,
	globals: IGlobals,
) {
...
export class ExecContext implements IExecContext {
	constructor(
		...
		public tree: Lisp[],
		public getSubscriptions: Set<
			(obj: SubscriptionSubject, name: string) => void
		>,
		public setSubscriptions: WeakMap<
			SubscriptionSubject,
			Map<string, Set<(modification: Change) => void>>
		>,
		public changeSubscriptions: WeakMap<
			SubscriptionSubject,
			Set<(modification: Change) => void>
		>,
...
export function createContext(
	...
	const context = {
		...
		prototypeWhitelist: new Map(
			[...options.prototypeWhitelist].map((a) => [a[0].prototype, a[1]]),
		),
		options,
		globalScope: new Scope(null, options.globals, sandboxGlobal),
		sandboxGlobal,
	};
	context.prototypeWhitelist.set(
...
export class Scope {
	...
	get(key: string, functionScope = false): Prop {
		...
		if (this.parent === null || !functionScope || functionThis !== undefined) {
			...
			if (
				key in this.allVars &&
				(!(key in {}) || this.allVars.hasOwnProperty(key))
			) {
				return new Prop(
					this.allVars,
					key,
	...
	declare(
		...
		if (
			type === "var" &&
			this.functionThis === undefined &&
			this.parent !== null
		) {
			return this.parent.declare(key, type, value, isGlobal);
		} else if (
			(this[type].hasOwnProperty(key) &&
				type !== "const" &&
				!this.globals.hasOwnProperty(key)) ||
			!(key in this.allVars)
		) {
...
export function isLisp<Type extends Lisp = Lisp>(
	item: LispItem | LispItem,
): item is Type {
	return (
		Array.isArray(item) &&
...
export class Prop {
	...
	get<T = unknown>(context: IExecContext): T {
		...
		if (ctx === null)
			throw new TypeError(
				`Cannot read properties of null, (reading '${this.prop}')`,
			);
