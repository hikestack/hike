...
export function assignCheck(obj: Prop, context: IExecContext, op = "assign") {
	...
	if (obj.isGlobal) {
		throw new SandboxError(
			`Cannot ${op} property '${obj.prop}' of a global object`,
		);
	}
	if (obj.context === null) {
	...
	if (
		typeof obj.context[obj.prop] === "function" &&
		!obj.context.hasOwnProperty(obj.prop)
	) {
		throw new SandboxError(
			`Override prototype property '${obj.prop}' not allowed`,
		);
	}
	if (op === "delete") {
...
export function addOps<Type extends LispFamily>(
	type: ExtractLispOp<Type>,
	cb: OpCallback,
) {
	ops.set(type, cb);
}
...
addOps(
	LispType.Prop,
	(exec, done, ticks, a, b: string, obj, context, scope) => {
		if (a === null) {
		...
		if (type === "undefined" && obj === undefined) {
			...
			if (prop.context === context.ctx.sandboxGlobal) {
				...
				const rep = context.ctx.globalsWhitelist.has(
					context.ctx.sandboxGlobal[b],
				)
		...
		const prototypeAccess =
			isFunction || !(a.hasOwnProperty(b) || typeof b === "number");
			...
			if (typeof b === "string") {
				...
				do {
					if (prot.hasOwnProperty(b)) {
						...
						context.ctx.auditReport?.prototypeAccess[prot.constructor.name].add(
							b,
						);
					}
		...
		if (prototypeAccess) {
			if (isFunction) {
				if (
					!["name", "length", "constructor"].includes(b) &&
					a.hasOwnProperty(b)
				) {
					...
					if (!(whitelist && (!whitelist.size || whitelist.has(b)))) {
						throw new SandboxError(
							`Static method or property access not permitted: ${a.name}.${b}`,
						);
					}
				}
				...
				while ((prot = Object.getPrototypeOf(prot))) {
					if (prot.hasOwnProperty(b)) {
						...
						const replace = context.ctx.options.prototypeReplacements.get(
							prot.constuctor,
						);
						if (replace) {
...
addOps(LispType.Call, (exec, done, ticks, a, b: Lisp[], obj, context) => {
	...
	if (
		obj.context[obj.prop] === JSON.stringify &&
		...
		if (obj.prop === "push") {
			...
			change = {
				...
				removed: obj.context.slice(
					vals[0],
					vals[1] === undefined ? undefined : vals[0] + vals[1],
				),
			};
...
addOps(
	LispType.CreateArray,
	(exec, done, ticks, a, b: Lisp[], obj, context) => {
		const items = b
			.map((item) => {
				if (item instanceof SpreadArray) {
					return [...item.item];
...
addOps(LispType.Number, (exec, done, ticks, a, b) =>
	done(undefined, Number(b)),
);
addOps(LispType.BigInt, (exec, done, ticks, a, b) =>
	done(undefined, BigInt(b)),
);
...
addOps(LispType.LargerThan, (exec, done, ticks, a, b) =>
	done(undefined, a > b),
);
addOps(LispType.SmallerThan, (exec, done, ticks, a, b) =>
	done(undefined, a < b),
);
addOps(LispType.LargerEqualThan, (exec, done, ticks, a, b) =>
	done(undefined, a >= b),
);
addOps(LispType.SmallerEqualThan, (exec, done, ticks, a, b) =>
	done(undefined, a <= b),
);
addOps(LispType.Equal, (exec, done, ticks, a, b) => done(undefined, a == b));
addOps(LispType.StrictEqual, (exec, done, ticks, a, b) =>
	done(undefined, a === b),
);
addOps(LispType.NotEqual, (exec, done, ticks, a, b) => done(undefined, a != b));
addOps(LispType.StrictNotEqual, (exec, done, ticks, a, b) =>
	done(undefined, a !== b),
);
addOps(LispType.And, (exec, done, ticks, a, b) => done(undefined, a && b));
addOps(LispType.Or, (exec, done, ticks, a, b) => done(undefined, a || b));
addOps(LispType.BitAnd, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a & b),
);
addOps(LispType.BitOr, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a | b),
);
addOps(LispType.Plus, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a + b),
);
addOps(LispType.Minus, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a - b),
);
...
addOps(LispType.Divide, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a / b),
);
addOps(LispType.BitNegate, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a ^ b),
);
addOps(LispType.Multiply, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a * b),
);
addOps(LispType.Modulus, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a % b),
);
addOps(LispType.BitShiftLeft, (exec, done, ticks, a: number, b: number) =>
	done(undefined, a << b),
);
...
addOps(
	LispType.Typeof,
	(exec, done, ticks, a, b: LispItem, obj, context, scope) => {
...
addOps(LispType.In, (exec, done, ticks, a: string, b) =>
	done(undefined, a in b),
);

addOps(
	LispType.Delete,
	(exec, done, ticks, a, b, obj, context, scope, bobj: Prop) => {
...
addOps(
	LispType.Var,
	(exec, done, ticks, a: string, b: LispItem, obj, context, scope) => {
...
addOps(
	LispType.Let,
	(exec, done, ticks, a: string, b: LispItem, obj, context, scope, bobj) => {
...
addOps(
	LispType.Loop,
	(exec, done, ticks, a: Lisp[], b: Lisp[], obj, context, scope) => {
		const [
			checkFirst,
			startInternal,
			getIterator,
		...
		if (exec === execAsync) {
			(async () => {
				...
				internalVars["$$obj"] =
					(ad = asyncDone((d) =>
						exec(ticks, getIterator, loopScope, context, d),
					)).isInstant === true
						? ad.instant
						: (await ad.p).result;
				ad = asyncDone((d) =>
					exec(ticks, startInternal, interalScope, context, d),
				);
				if (checkFirst)
					loop =
						(ad = asyncDone((d) =>
							exec(ticks, condition, interalScope, context, d),
						)).isInstant === true
							? ad.instant
							: (await ad.p).result;
				while (loop) {
					...
					ad = asyncDone((d) =>
						exec(
							ticks,
							beforeStep,
							new Scope(interalScope, innerLoopVars),
							context,
							d,
						),
					);
...
addOps(LispType.InlineIfCase, (exec, done, ticks, a, b) =>
	done(undefined, new If(a, b)),
);
addOps(LispType.IfCase, (exec, done, ticks, a, b) =>
	done(undefined, new If(a, b)),
);

addOps(
	LispType.Switch,
	(exec, done, ticks, a: LispItem, b: SwitchCase[], obj, context, scope) => {
				...
				for (const caseItem of b) {
					if (
						...
						(isTrue =
							...
							toTest ===
								valueOrProp(
									syncDone((d) => exec(ticks, caseItem[1], scope, context, d))
										.result,
									context,
								))
					) {
						...
						if (
							...
							(isTrue =
								...
								toTest ===
									valueOrProp(
										(ad = asyncDone((d) =>
											exec(ticks, caseItem[1], scope, context, d),
										)).isInstant === true
											? ad.instant
											: (await ad.p).result,
										context,
...
addOps(
	...
	(
		...
		if (!context.ctx.globalsWhitelist.has(a) && !sandboxedFunctions.has(a)) {
			throw new SandboxError(
				`Object construction not allowed: ${a.constructor.name}`,
			);
		}
...
addOps(LispType.Expression, (exec, done, ticks, a: unknown[]) =>
	done(undefined, a.pop()),
);
...
export function execMany(
	...
	if (exec === execSync) {
		...
		_execManyAsync(ticks, tree, done, scope, context, inLoopOrSwitch).catch(
			done,
		);
	}
}
...
function _execManySync(
	...
	for (let i = 0; i < tree.length; i++) {
		...
		try {
			res = syncDone((d) =>
				execSync(ticks, tree[i], scope, context, d, inLoopOrSwitch),
			).result;
		} catch (e) {
			done(e);
			return;
		...
		if (
			res instanceof ExecReturn &&
			(res.returned || res.breakLoop || res.continueLoop)
		) {
			done(undefined, res);
			return;
		}
...
async function _execManyAsync(
	...
	for (let i = 0; i < tree.length; i++) {
		...
		try {
			...
			res =
				(ad = asyncDone((d) =>
					execAsync(ticks, tree[i], scope, context, d, inLoopOrSwitch),
				)).isInstant === true
					? ad.instant
					: (await ad.p).result;
		} catch (e) {
...
export async function execAsync<T = any>(
	...
	if (
		...
		try {
			...
			obj =
				(ad = asyncDone((d) =>
					execAsync(ticks, tree[1], scope, context, d, inLoopOrSwitch),
				)).isInstant === true
					? ad.instant
					: (await ad.p).result;
		} catch (e) {
		...
		try {
			...
			bobj =
				(ad = asyncDone((d) =>
					execAsync(ticks, tree[2], scope, context, d, inLoopOrSwitch),
				)).isInstant === true
					? ad.instant
					: (await ad.p).result;
		} catch (e) {
...
export function execSync<T = any>(
	...
	if (
		...
		try {
			obj = syncDone((d) =>
				execSync(ticks, tree[1], scope, context, d, inLoopOrSwitch),
			).result;
		} catch (e) {
			done(e);
			return;
		...
		try {
			bobj = syncDone((d) =>
				execSync(ticks, tree[2], scope, context, d, inLoopOrSwitch),
			).result;
		} catch (e) {
			done(e);
			return;
...
function _execNoneRecurse<T = any>(
	...
	if (
		context.ctx.options.executionQuota &&
		context.ctx.options.executionQuota <= ticks.ticks
	) {
		if (
			!(
	...
	if (tree instanceof Prop) {
		...
		if (!isAsync) {
				...
				new SandboxError(
					"Illegal use of 'await', must be inside async function",
				),
			);
